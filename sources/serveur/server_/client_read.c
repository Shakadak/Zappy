...
char					*check_cases_h(t_env *e, int y, int x, int hm)
{
	...
	xc = hm - 1;
	count = 1;
	res = ft_strclone(lookoncase(e, y, x - hm, (hm * hm)));
	res = ft_stradd(res, ", ");
	while (xc > 0)
	{
		res = ft_stradd(res, lookoncase(e, y, x - xc, (hm * hm) + count));
		res = ft_stradd(res, ", ");
		xc++;
		count++;
	}
...
char					*check_cases_v(t_env *e, int y, int x, int hm)
{
	...
	ft_putstr("<");
	count = 1;
	res = ft_strclone(lookoncase(e, y  - hm, x, (hm * hm)));
	res = ft_stradd(res, ", ");
	while (yc > 0)
	{
		res = ft_stradd(res, lookoncase(e, y - yc, x, (hm * hm) + count));
		res = ft_stradd(res, ", ");
		yc++;
		count++;
	}
...
char					*seecaselvl(int lvl, int pl, t_env *e)
{
	...
	if (((e->pl[pl].dir % 4) * (M_PI / 2)) == UP)
		return (check_cases_h(e, ((e->pl[pl].y + lvl + 1)
			% e->map.ylen), e->pl[pl].x, lvl + 1));
	if (((e->pl[pl].dir % 4) * (M_PI / 2)) == DOWN)
		return (check_cases_h(e, ((e->pl[pl].y - lvl - 1)
			% e->map.ylen), e->pl[pl].x, lvl + 1));
	if (((e->pl[pl].dir % 4) * (M_PI / 2)) == LEFT)
		return (check_cases_v(e, e->pl[pl].y, ((e->pl[pl].x - 1 - lvl)
			% e->map.xlen), lvl + 1));
...
int						getdir(int x, int y, t_env *e, int pl)
{
	int					res;
	double				tmpx;
	double				tmpy;
	...
	if (norm > 0)
	{
		tmpx = (double)x / (norm * (-1));
		tmpy = (double)y / (norm * (-1));
		ft_putstr("\tGETDIR");
		ft_putstr("\tDIRPL:");
		ft_putstr(ft_itoa(e->pl[pl].dir));
		ft_putstr("\t\tXVEC:");
		...
		tmpx = 0;
		if (e->pl[pl].dir < 0)
			e->pl[pl].dir = e->pl[pl].dir + 4;
		while (res < 8)
		{
			if (angl >= ((((M_PI * 2) / 8) * res) - (((e->pl[pl].dir % 4) * (M_PI / 2))))
				&& angl <= ((((M_PI * 2) / 8) * (res + 1)) - ((e->pl[pl].dir % 4) * (M_PI / 2))))
				return (res + 1);
			res++;
		}
	}
...
int						get_opt_(char *buf, t_env *e, int cs)
{
	...
	tmp = (char *)malloc(sizeof(char) * lenuntil(buf, ' '));
	i = 0;
	while (buf[i] != '\0')
	{
		tmp[i] = buf[i];
